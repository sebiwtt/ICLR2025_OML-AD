...
def calculate_anomaly_scores(forecast, ground_truth_values):
    ...
    for i, true_value in enumerate(ground_truth_values):
        ...
        threshold = np.abs(prediction-upper_bound) * 4
...
def train_model(df_prophet, training_set_size):
    ...
    future = model.make_future_dataframe(periods=len(remainder), freq='D')
...
def train_and_evaluate(df):
    ...
    no_retraining_forecast = train_model(df_prophet, 200)
